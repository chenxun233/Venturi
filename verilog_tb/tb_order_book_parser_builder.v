...
`include "../market_data/HSBC_3816_S_payload_frames_hex.txt"
    ...
    always @(posedge i_clk_156) begin
        ...
        if (o_builder_valid) begin
            ...
            if (batch_checks_en && batch_payloads == 0 && payload_stock_locate != AAPL_LOCATE) begin
                $display("[%0t] ERROR: first arbitrated payload in batch was not AAPL", $time);
                errors <= errors + 1;
            end
            if (batch_checks_en && batch_payloads == 1 && payload_stock_locate != HSBC_LOCATE) begin
    ...
    initial begin
        ...
        if (errors != 0) begin
            $fatal(1, "Parser/builder arbiter test failed with %0d errors", errors);
        end
