...
Align the runtime app names and the server default listen address for the FPGA boost demo so the two-process demo matches the intended physical setup:
...
- the server TCP module change needed to make the default listen address `192.168.50.2`
...
- Any other connect-related modification in the TX TCP module or server TCP module
...
The intended runtime behavior is that `dummy_server` can be launched without any extra address arguments and still bind to the correct interface-facing address. The previous loopback default is removed from the expected runtime path.

### 3. Client TCP behavior

Do not modify the existing client connection behavior. The dedicated `TxEngine` thread remains unchanged, and `Executor` still pushes records into the TX path exactly as it does now.
...
The TCP behavior remains structurally unchanged except for the server default listen address:
...
- No new server launch arguments are required for address selection.
...
4. Start `dummy_server` with no extra address arguments and verify it reports `192.168.50.2:9000`.
...
- Prefer changing the server default constant and target names rather than introducing new abstractions.
