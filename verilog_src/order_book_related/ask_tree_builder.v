module ask_tree_builder #(
    ...
    reg                          bottom_ask_t_valid [0:LAST_COUNT-1];
    reg [QTY_PRICE_LVL_BIT-1:0]  mid_ask_t_idx    [0:MID_COUNT-1];
    reg                          mid_ask_t_valid  [0:MID_COUNT-1];

    wire [QTY_PRICE_LVL_BIT-1:0] mid_to_bottom_ask_t_idx   [MID_LEVEL:LEVELS-1][0:UPPER_MAX_COUNT-1];
    wire                         mid_to_bottom_ask_t_valid [MID_LEVEL:LEVELS-1][0:UPPER_MAX_COUNT-1];
    wire [QTY_PRICE_LVL_BIT-1:0] top_to_mid_ask_t_idx    [0:LOWER_LEVELS-1][0:LOWER_MAX_COUNT-1];
    wire                         top_to_mid_ask_t_valid  [0:LOWER_LEVELS-1][0:LOWER_MAX_COUNT-1];

    integer idx;

    // The leaf level stores only valid bits. The index is implied by the array subscript.
    // mid_ask_t_* is the pipeline register stage at MID_LEVEL.
    always @(posedge i_clk or posedge i_rst) begin
        if (i_rst) begin
            for (idx = 0; idx < LAST_COUNT; idx = idx + 1) begin
                bottom_ask_t_valid[idx] <= 1'b0;
            end
            for (idx = 0; idx < MID_COUNT; idx = idx + 1) begin
                mid_ask_t_valid[idx] <= 1'b0;
            ...
            for (idx = 0; idx < MID_COUNT; idx = idx + 1) begin
                mid_ask_t_valid[idx] <= mid_to_bottom_ask_t_valid[MID_LEVEL][idx];
                mid_ask_t_idx[idx]   <= mid_to_bottom_ask_t_idx[MID_LEVEL][idx];
            end
        end
    end
    ...
    generate
        ...
        for (level = MID_LEVEL; level < LEVELS; level = level + 1) begin : gen_bottom_to_mid_levels
            localparam integer NODE_COUNT = (1 << level);
            for (node = 0; node < NODE_COUNT; node = node + 1) begin : gen_bottom_to_mid_nodes
