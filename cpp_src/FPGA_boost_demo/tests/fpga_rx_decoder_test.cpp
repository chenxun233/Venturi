...
FakeFPGADev::RawSlot makeRawSlot(uint16_t stock_locate,
                                 uint64_t event_logic_latency_tk,
    ...
    writeLe48(slot, 2, event_logic_latency_tk);
...
TEST(FpgaRxDecoderTest, decodesKnownRawBytesIntoExpectedFields) {
    ...
    const std::size_t count = decoder.decodeRawBatch(out.data(), out.size());

    ASSERT_EQ(count, 1U);
    EXPECT_EQ(out[0].stock_locate, 0x000d);
    EXPECT_EQ(out[0].event_logic_latency_tk, 0x000102030405ULL);
...
TEST(FpgaRxDecoderTest, decodesSyncSnapshotAndPublishesConsumerProgress) {
    ...
    const std::size_t count = decoder.decodeRawBatchSync(out.data(), snapshot, true, out.size());
...
TEST(FpgaRxDecoderTest, constructsDecoderWithFakeSourcePattern) {
    ...
    fake_device.setRawSlots(0, {makeRawSlot(0x0011, 1U, 2U, 3U, 4U, 5U, 6U)});
    fake_device.setProdPtr(0, 1U);

    FPGARxDecoder decoder(fake_device, 0);
    std::array<FPGAEventDesc, 1> out {};

    EXPECT_EQ(decoder.decodeRawBatch(out.data(), out.size()), 1U);
}
