...
class FixedRingBuffer {
    static_assert(Capacity != 0 && (Capacity & (Capacity - 1)) == 0,
                  "FixedRingBuffer capacity must be a non-zero power of two");
...
public:
    ...
    bool        copyFront(T* out, std::size_t count) const;
...
template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::isEmpty() const {
    return m_count == 0;
}

template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::_isFull() const {
    return m_count == Capacity;
}

template <typename T, std::size_t Capacity>
std::size_t FixedRingBuffer<T, Capacity>::readSize() const {
    return m_count;
}

template <typename T, std::size_t Capacity>
constexpr std::size_t FixedRingBuffer<T, Capacity>::readCapacity() const {
    return Capacity;
}

template <typename T, std::size_t Capacity>
void FixedRingBuffer<T, Capacity>::clear() {
    m_head = 0;
    m_count = 0;
}

template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::pushBack(const T& value) {
    if (_isFull()) {
        return false;
    }
...
template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::pushBack(T&& value) {
    if (_isFull()) {
        return false;
    }
...
template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::write(const T* values, std::size_t count) {
...
template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::eraseFront() {
...
template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::eraseFrontN(std::size_t count) {
    if (count > m_count) {
        return false;
    }
    if (count == 0) {
...
template <typename T, std::size_t Capacity>
T& FixedRingBuffer<T, Capacity>::readFront() {
    return m_records[m_head];
}

template <typename T, std::size_t Capacity>
const T& FixedRingBuffer<T, Capacity>::readFront() const {
    return m_records[m_head];
}

template <typename T, std::size_t Capacity>
T& FixedRingBuffer<T, Capacity>::readAt(std::size_t offset) {
    return m_records[_wrapIndexP1(m_head + offset)];
}

template <typename T, std::size_t Capacity>
const T& FixedRingBuffer<T, Capacity>::readAt(std::size_t offset) const {
    return m_records[_wrapIndexP1(m_head + offset)];
}

template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::copyFront(T* out, std::size_t count) const {
    return copyFrom(0, out, count);
}

template <typename T, std::size_t Capacity>
bool FixedRingBuffer<T, Capacity>::copyFrom(std::size_t offset, T* out, std::size_t count) const {
...
template <typename T, std::size_t Capacity>
std::size_t FixedRingBuffer<T, Capacity>::_wrapIndexP1(std::size_t idx) const {
